...
class QobuzDL {
    ...
    async testCorsProxy () {
        try {
            const response = await fetch(`https://cors.kvtdev.me/?u=https://httpbin.org/status/200`);
    ...
    async getAppIdAndSecrets() {
        try {
            ...
            const response = await fetch(`https://cors.kvtdev.me/?u=${encodeURIComponent("https://play.qobuz.com/login")}`);
            ...
            const bundleResponse = await fetch(`https://cors.kvtdev.me/?u=${encodeURIComponent("https://play.qobuz.com" + bundleUrl)}`);
